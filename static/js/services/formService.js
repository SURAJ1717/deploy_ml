...
app.service('formService', ['$http', '$rootScope', 
    ...
    function ($http, $rootScope) {
        ...
        fs.openTab = function(tab_name){

            if(tab_name == 'linear_tab'){
                fs.linear_tab = true;
                fs.ridge_tab = fs.lasso_tab = fs.dt_tab = fs.rf_tab = fs.xgb_tab = false;
            }

            if(tab_name == 'ridge_tab'){
                fs.ridge_tab = true;
                fs.linear_tab = fs.lasso_tab = fs.dt_tab = fs.rf_tab = fs.xgb_tab = false;
            }

            if(tab_name == 'lasso_tab'){
                fs.lasso_tab = true;
                fs.linear_tab = fs.ridge_tab = fs.dt_tab = fs.rf_tab = fs.xgb_tab = false;
            }

            if(tab_name == 'dt_tab'){
                fs.dt_tab = true;
                fs.linear_tab = fs.ridge_tab = fs.lasso_tab = fs.rf_tab = fs.xgb_tab = false;
            }

            if(tab_name == 'rf_tab'){
                fs.rf_tab = true;
                fs.linear_tab = fs.ridge_tab = fs.lasso_tab = fs.dt_tab = fs.xgb_tab = false;
            }

            if(tab_name == 'xgb_tab'){
                fs.xgb_tab = true;
                fs.linear_tab = fs.ridge_tab = fs.lasso_tab = fs.dt_tab = fs.rf_tab = false;
            }
        }

        fs.appendAlgoParamInputs = function(current_val, algo_type, hyper_param){

            if(algo_type == 'ridge_params'){

                if(hyper_param == 'alpha'){

                    var exist = fs.ridge_params.alpha.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.ridge_params.alpha.push(current_val);
                    }
                }
            }

            if(algo_type == 'lasso_params'){

                if(hyper_param == 'alpha'){

                    var exist = fs.lasso_params.alpha.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.lasso_params.alpha.push(current_val);
                    }
                }
            }

            if(algo_type == 'dt_params'){

                if(hyper_param == 'splitter'){

                    var exist = fs.dt_params.splitter.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.splitter.push(current_val);
                    }
                }

                if(hyper_param == 'max_depth'){

                    var exist = fs.dt_params.max_depth.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.max_depth.push(current_val);
                    }
                }

                if(hyper_param == 'min_samples_leaf'){

                    var exist = fs.dt_params.min_samples_leaf.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.min_samples_leaf.push(current_val);
                    }
                }

                if(hyper_param == 'min_weight_fraction_leaf'){

                    var exist = fs.dt_params.min_weight_fraction_leaf.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.min_weight_fraction_leaf.push(current_val);
                    }
                }

                if(hyper_param == 'max_features'){

                    var exist = fs.dt_params.max_features.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.max_features.push(current_val);
                    }
                }

                if(hyper_param == 'max_leaf_nodes'){

                    var exist = fs.dt_params.max_leaf_nodes.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.dt_params.max_leaf_nodes.push(current_val);
                    }
                }
            }

            if(algo_type == 'rf_params'){

                if(hyper_param == 'n_estimators'){

                    var exist = fs.rf_params.n_estimators.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.rf_params.n_estimators.push(current_val);
                    }
                }

                if(hyper_param == 'min_samples_split'){

                    var exist = fs.rf_params.min_samples_split.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.rf_params.min_samples_split.push(current_val);
                    }
                }

                if(hyper_param == 'max_features'){

                    var exist = fs.rf_params.max_features.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.rf_params.max_features.push(current_val);
                    }
                }

                if(hyper_param == 'max_depth'){

                    var exist = fs.rf_params.max_depth.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.rf_params.max_depth.push(current_val);
                    }
                }

                if(hyper_param == 'min_samples_leaf'){

                    var exist = fs.rf_params.min_samples_leaf.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.rf_params.min_samples_leaf.push(current_val);
                    }
                }
            }

            if(algo_type == 'xgb_params'){

                if(hyper_param == 'n_estimators'){

                    var exist = fs.xgb_params.n_estimators.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.xgb_params.n_estimators.push(current_val);
                    }
                }

                if(hyper_param == 'learning_rate'){

                    var exist = fs.xgb_params.learning_rate.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.xgb_params.learning_rate.push(current_val);
                    }
                }

                if(hyper_param == 'min_child_weight'){

                    var exist = fs.xgb_params.min_child_weight.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.xgb_params.min_child_weight.push(current_val);
                    }
                }

                if(hyper_param == 'max_depth'){

                    var exist = fs.xgb_params.max_depth.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.xgb_params.max_depth.push(current_val);
                    }
                }

                if(hyper_param == 'subsample'){

                    var exist = fs.xgb_params.subsample.indexOf(current_val);

                    if( exist == -1 && current_val != ''){

                        fs.xgb_params.subsample.push(current_val);
                    }
                }
            }
        }

        fs.resetAlgoParamInputs = function(algo_type, hyper_param){

            if(algo_type == 'ridge_params'){

                if(hyper_param == 'alpha'){

                    fs.ridge_params.alpha=[]; 
                }
            }
            ...
            if(algo_type == 'dt_params'){
                ...
                if(hyper_param == 'splitter'){
                    ...
                    fs.dt_params.splitter=[]; 
                }

                if(hyper_param == 'max_depth'){

                    fs.dt_params.max_depth=[]; 
                }

                if(hyper_param == 'min_samples_leaf'){

                    fs.dt_params.min_samples_leaf=[]; 
                }

                if(hyper_param == 'min_weight_fraction_leaf'){

                    fs.dt_params.min_weight_fraction_leaf=[]; 
                }

                if(hyper_param == 'max_features'){

                    fs.dt_params.max_features=[]; 
                }

                if(hyper_param == 'max_leaf_nodes'){

                    fs.dt_params.max_leaf_nodes=[]; 
                }
            }
            ...
            if(algo_type == 'rf_params'){
                ...
                if(hyper_param == 'n_estimators'){
                    ...
                    fs.rf_params.n_estimators=[]; 
                }

                if(hyper_param == 'min_samples_split'){

                    fs.rf_params.min_samples_split=[]; 
                }

                if(hyper_param == 'max_features'){

                    fs.rf_params.max_features=[]; 
                }

                if(hyper_param == 'max_depth'){

                    fs.rf_params.max_depth=[]; 
                }

                if(hyper_param == 'min_samples_leaf'){

                    fs.rf_params.min_samples_leaf=[]; 
                }
            }
            ...
            if(algo_type == 'xgb_params'){
                ...
                if(hyper_param == 'n_estimators'){
                    ...
                    fs.xgb_params.n_estimators=[]; 
                }

                if(hyper_param == 'learning_rate'){

                    fs.xgb_params.learning_rate=[]; 
                }

                if(hyper_param == 'min_child_weight'){

                    fs.xgb_params.min_child_weight=[]; 
                }

                if(hyper_param == 'max_depth'){

                    fs.xgb_params.max_depth=[]; 
                }

                if(hyper_param == 'subsample'){

                    fs.xgb_params.subsample=[]; 
                }
            }
        }
